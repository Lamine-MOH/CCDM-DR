...
Model selection is done on a stratified VALIDATION split carved from the real
train h5 (--val_frac 0.12, --val_seed 999, identical across arms/seeds); the
val-selected weights are evaluated ONCE on the held-out test set at the end.
Synthetic images are concatenated with the real TRAIN set only -- they never
appear in the validation split, so val QWK stays a fair cross-arm selector.
...
def load_h5(path):
    ...
    if images.dtype != np.uint8:
        raise ValueError(f"{path}: images dtype {images.dtype} != uint8")
    if images.ndim != 4 or images.shape[1] != 3:
        raise ValueError(f"{path}: expected (N,3,H,W) uint8, got {images.shape}")
    bad = np.unique(labels)
    bad = bad[(bad < 0) | (bad > 4)]
    if len(bad):
        raise ValueError(f"{path}: labels outside [0,4]: {bad}")
    return images, labels


def cap_per_class(images, labels, cap, rng=None):
    if cap is None:
        return images, labels
    if rng is None:
    ...
    for g in np.unique(labels):
        ...
        if len(idx_g) > cap:
            idx_g = rng.choice(idx_g, size=cap, replace=False)
...
def stratified_val_split(images, labels, val_frac, val_seed):
    rng = np.random.default_rng(val_seed)
    tr_i, va_i = [], []
    for g in np.sort(np.unique(labels)):
        idx = np.where(labels == g)[0]
        rng.shuffle(idx)
        n_val = max(1, min(int(round(val_frac * len(idx))), len(idx) - 1))
        va_i.append(idx[:n_val])
        tr_i.append(idx[n_val:])
    tr_i = np.concatenate(tr_i)
    va_i = np.concatenate(va_i)
    return images[tr_i], labels[tr_i], images[va_i], labels[va_i]
...
def main():
    ...
    p.add_argument("--synthetic_h5", type=str, default=None,
                   help="images generated by CCDM via generate_from_ckpt.py (NOT --dump_fake_data)")
    ...
    p.add_argument("--backbone", type=str, default="resnet50",
                   choices=["resnet50", "resnet101", "efficientnet_b3", "efficientnet_b4",
                            "efficientnet_b5", "densenet121", "densenet201", "vit_base_patch14_dinov2"])
    p.add_argument("--pretrained", action=argparse.BooleanOptionalAction, default=True,
                   help="use ImageNet pretrained weights (default); pass --no-pretrained to train from scratch")
    ...
    p.add_argument("--val_frac", type=float, default=0.12,
                   help="fraction of real train h5 held out for val-based model selection (0 = legacy test-selection)")
    p.add_argument("--val_seed", type=int, default=999, help="seed for the stratified val split (identical across arms)")
    ...
    # Val-based model selection: carve a balanced val split from the real
    # train h5; the val split is fixed across all arms/seeds (val_seed).
    if args.val_frac > 0:
        train_images, train_labels, val_images, val_labels = stratified_val_split(
            real_images, real_labels, args.val_frac, args.val_seed)
        print(f"Val split: carved {args.val_frac:.0%} from real train set (val_seed={args.val_seed})")
    else:
        print("WARNING (deprecated): --val_frac 0 selects on the TEST set. "
              "Use --val_frac 0.12 per the protocol.")
        train_images, train_labels = real_images, real_labels
        val_images, val_labels = test_images, test_labels

    # Concat synthetic with TRAIN set only — val never sees synthetic.
    if args.synthetic_h5 is not None:
        syn_images, syn_labels = load_h5(args.synthetic_h5)
        syn_images, syn_labels = cap_per_class(
            syn_images, syn_labels, args.synthetic_cap_per_grade,
        ...
        print(f"Added {len(syn_labels)} synthetic images. Final training-set grade distribution:")
    else:
        print("Training-set grade distribution (real only):")
    for g in range(5):
        print(f"  grade {g}: {int((train_labels == g).sum())}")
    print(f"Val-set grade distribution:")
    for g in range(5):
        print(f"  grade {g}: {int((val_labels == g).sum())}")

    train_ds = DRDataset(train_images, train_labels, train=True, img_size=args.img_size)
    val_ds = DRDataset(val_images, val_labels, train=False, img_size=args.img_size)
    ...
    best_state = torch.load(os.path.join(args.out_dir, f"{args.run_name}_best.pth"), map_location=device)
    ...
    if args.save_test_preds is not None:
        ...
        print(f"Saved best-model test predictions -> {args.save_test_preds}")

    selection = "test" if args.val_frac <= 0 else "val"
    test_metrics["run_name"] = args.run_name
    test_metrics["backbone"] = args.backbone
    test_metrics["pretrained"] = bool(args.pretrained)
    test_metrics["epochs"] = args.epochs
    test_metrics["batch_size"] = args.batch_size
    test_metrics["lr"] = args.lr
    test_metrics["seed"] = args.seed
    test_metrics["val_seed"] = args.val_seed
    test_metrics["val_frac"] = args.val_frac
    ...
    print(f"\nBest val QWK for '{args.run_name}': {best_val_qwk:.4f} "
          f"(epoch {best_epoch}). Test QWK: {test_metrics['qwk']:.4f}. Saved {out_path}")
