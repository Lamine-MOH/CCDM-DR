...
from models import ResNet34_embed_y2h, model_y2h, ResNet34_embed_y2cov, model_mlp_y2cov, model_cnn_y2cov
...
class LabelEmbed:
    def __init__(
        ...
        self.y2cov_y2emb_type = y2cov_y2emb_type
        self.h_dim = h_dim 
        ...
        assert y2cov_y2emb_type in ['mlp', 'cnn']
        ...
        if y2h_type == "resnet":
            ...
            labels_tmp = torch.from_numpy(labels_tmp).type(torch.float).to(self.device)
            epsilons_tmp = np.random.normal(0, 0.2, len(labels_tmp))
            epsilons_tmp = torch.from_numpy(epsilons_tmp).view(-1,1).type(torch.float).to(self.device)
        ...
        if y2cov_type == "resnet":
            ...
            y2emb_y2cov_filename_ckpt = os.path.join(self.path_y2cov, 'ckpt_{}_y2cov_epoch_{}.pth'.format(self.y2cov_y2emb_type, epochs_y2cov))
            
            # init network
            model_resnet_y2cov = ResNet34_embed_y2cov(img_size=self.img_size, nc=self.nc)
            model_resnet_y2cov = model_resnet_y2cov.to(device)
            model_resnet_y2cov = nn.DataParallel(model_resnet_y2cov)
            
            if self.y2cov_y2emb_type == "mlp":
                model_y2emb_y2cov = model_mlp_y2cov(img_size=self.img_size, nc=self.nc)
            else:
                model_y2emb_y2cov = model_cnn_y2cov(img_size=self.img_size, nc=self.nc)
            model_y2emb_y2cov = model_y2emb_y2cov.to(device)
            model_y2emb_y2cov = nn.DataParallel(model_y2emb_y2cov)
            ...
            if not os.path.isfile(y2emb_y2cov_filename_ckpt):
                ...
                }, y2emb_y2cov_filename_ckpt)
            else:
                print("\n model y2emb_y2cov ckpt already exists")
                print("\n Loading...")
                checkpoint = torch.load(y2emb_y2cov_filename_ckpt, weights_only=True)
                model_y2emb_y2cov.load_state_dict(checkpoint['net_state_dict'])
            #end not os.path.isfile
            
            self.model_y2emb_y2cov = model_y2emb_y2cov
            ...
            model_y2emb_y2cov.eval()
            with torch.no_grad():
                labels_hidden_tmp = model_y2emb_y2cov(labels_tmp)
                labels_noise_hidden_tmp = model_y2emb_y2cov(labels_noise_tmp)
                labels_rec_tmp = net_h2y(labels_hidden_tmp).cpu().numpy().reshape(-1,1)
                labels_noise_rec_tmp = net_h2y(labels_noise_hidden_tmp).cpu().numpy().reshape(-1,1)
                labels_hidden_tmp = labels_hidden_tmp.cpu().numpy()
                labels_noise_hidden_tmp = labels_noise_hidden_tmp.cpu().numpy()
    ...
    def fn_y2h(self, labels):
        ...
        if self.y2h_type == "sinusoidal":
            ...
            self.model_mlp_y2h = self.model_mlp_y2h.to(self.device)
    ...
    def fn_y2cov(self, labels):
        ...
        if self.y2cov_type == "sinusoidal":
            ...
            self.model_y2emb_y2cov.eval()
            self.model_y2emb_y2cov = self.model_y2emb_y2cov.to(self.device)
            embedding = self.model_y2emb_y2cov(labels)
...
def train_y2emb(unique_labels_norm, model_y2emb, model_name, model_h2y, epochs=500, lr_base=0.01, lr_decay_factor=0.1, lr_decay_epochs=[150, 250, 350], weight_decay=1e-4, batch_size=128, device="cuda"):
    '''
    unique_labels_norm: an array of normalized unique labels
    '''

    model_y2emb = model_y2emb.to(device)
    ...
    model_h2y.eval()
    optimizer = torch.optim.SGD(model_y2emb.parameters(), lr = lr_base, momentum= 0.9, weight_decay=weight_decay)

    start_tmp = timeit.default_timer()
    for epoch in range(epochs):
        model_y2emb.train()
        train_loss = 0
        adjust_learning_rate_2(optimizer, epoch)
        for _, batch_labels in enumerate(trainloader):
            ...
            #Forward pass
            batch_hiddens_noise = model_y2emb(batch_labels_noise)
