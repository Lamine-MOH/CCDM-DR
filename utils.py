...
def cal_std_dask(large_array, chunk_size=100000):
    """
    Calculate the normalized variance of a large array using Dask
    """
    # Convert numpy array to dask array
    dask_array = da.from_array(large_array, chunks=chunk_size)
    
    # Normalize to [-1, 1]
    normalized = (dask_array.astype(np.float64) / 255.0 - 0.5) / 0.5
    
    # Calculate variance (Dask will automatically handle chunking)
    std_dev = da.std(normalized).compute()
