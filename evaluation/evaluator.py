...
class Evaluator:
    def __init__(self, dataset, trainer, args, device, save_results_folder):
        ...
        elif self.data_name=="DRGrading":
            # NOTE: unlike the released datasets, there are no pretrained SFID/label-score
            # networks shipped for DRGrading. You must train your own AE (for the SFID
            # feature space) and ResNet34 classifier/regressor on the DR training split
            # before --do_eval will work here (see evaluation/train_dr_eval_nets.py and
            # the acknowledged CcGAN-AVAR repo for the training recipe). Until then, run
            # with --do_eval disabled and rely on downstream_eval/ for utility-based
            # evaluation instead.
            self.num_classes = 5
            self.nfake_per_label = 1000
            self.nburnin_per_label = 2000
            self.fid_net_path = "./evaluation/eval_ckpts/DRGrading/metrics_{}x{}/ckpt_AE_epoch_200_seed_2020_CVMode_False.pth".format(args.image_size, args.image_size)
            self.class_net_path = "./evaluation/eval_ckpts/DRGrading/metrics_{}x{}/ckpt_PreCNNForEvalGANs_ResNet34_class_epoch_200_seed_2020_classify_5_grades_CVMode_False.pth".format(args.image_size, args.image_size)
            self.regre_net_path = "./evaluation/eval_ckpts/DRGrading/metrics_{}x{}/ckpt_PreCNNForEvalGANs_ResNet34_regre_epoch_200_seed_2020_CVMode_False.pth".format(args.image_size, args.image_size)
        else:
