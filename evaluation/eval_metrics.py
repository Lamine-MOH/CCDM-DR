...
def cal_FID(PreNetFID, IMGSr, IMGSg, batch_size = 500, resize = None, norm_img = False, device="cuda"):
...
##############################################################################
def cal_labelscore(PreNet, images, labels_assi, min_label_before_shift, max_label_after_shift, batch_size = 200, resize = None, norm_img = False, num_workers=0, device="cuda"):
    '''
...
##############################################################################
def inception_score(imgs, num_classes, net, batch_size=32, splits=1, normalize_img=False, device="cuda"):
...
def predict_class_labels(net, images, batch_size=500, verbose=False, num_workers=0, device="cuda"):
    net = net.to(device)
    net.eval()
