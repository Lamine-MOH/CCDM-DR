...
class LoadDataSet:
    def __init__(self, data_name, data_path, min_label, max_label, img_size=64, max_num_img_per_label=1e30, num_img_per_label_after_replica=0, imbalance_type="unimodal"):
        """
        data_name: the name of the dataset; must be one of ['RC-49', 'Cell200', 'UTKFace', 'SteeringAngle', 'DRGrading']
        ...
        with h5py.File(self.data_path+'/{}_{}x{}.h5'.format(self.data_name, self.img_size, self.img_size), 'r') as hf:
            if self.data_name == "RC-49":
                ...
                ## load h5 file; labels are integer DR severity grades (0=No DR ... 4=Proliferative DR)
                self.labels_all = hf['labels'][:].astype(float)
                self.images_all = hf['images'][:]
                hf.close()
                print("\n Loaded entire DRGrading dataset: {}x{}x{}x{}".format(self.images_all.shape[0], self.images_all.shape[1], self.images_all.shape[2], self.images_all.shape[3]))
                self.num_classes = 5  # ICDR scale: 0,1,2,3,4
    ...
    def load_train_data(self):
        ...
        elif self.data_name in ["UTKFace", "DRGrading"]:
            ## Extract a subset from the entire dataset.
            ## (DRGrading reuses the UTKFace logic: both have small integer label sets,
            ## and DR datasets are typically severely imbalanced across grades, so the
            ## minority-class replication below is directly useful here.)
            images = []
            labels = []
    ...
    def load_evaluation_data(self):
        ...
        elif self.data_name in ["UTKFace", "Cell200", "DRGrading"]:
            ## Extract a subset from the entire dataset.
            images = []
            labels = []
            selected_labels = np.arange(self.min_label, self.max_label+1)
...
if __name__ == "__main__":
    ...
    p.add_argument("--data_name", default="RC-49")
    p.add_argument("--data_path", default="./data/RC-49")
    p.add_argument("--min_label", type=float, default=0)
    p.add_argument("--max_label", type=float, default=90)
